...
class NeuralNetwork(object):
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            delta_weights_i_h += np.dot(X.reshape(1, X.shape[0]).T, hidden_error_term)
            # Weight step (hidden to output)
            delta_weights_h_o += hidden_outputs.reshape(1, hidden_outputs.shape[0]).T * output_error_term
...
# ## 模型对数据预测的效果比较一般，验证集误差在0.16左右浮动，对测试集的预测也不是太好。我认为可能是超参数设置中出现了问题。
